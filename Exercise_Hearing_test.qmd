...
The dataset is stored as a Microsoft Excel file (.xlsx) within the following directory: *https://pub.ww.ovgu.de/lichters/smpi/data/hearing_test.xlsx*
...
We can see that the dataset includes the following variables:

-   **Gender**, indicating the respondent's gender

-   **Age** in years

-   **Frequency**, indicating the highest perceivable frequency in Hertz

-   **Year**, indicating the year of data collection
:::
...
Indeed, it seems that the increasing age is deteriorating hearing capabilities in terms of the highest perceivable frequency of a tone in Hertz. At least, the data is supporting the hypothesis when the regression coefficient is negative and the corresponding p-value is below the conventional threshold of 5%.
:::

# Task 2 {#sec-linear-regression-interaction}

> Is this process contingent on gender? Use the PROCESS Macro’s (Hayes, 2022) model 1 to evaluate whether age in years interacts with gender in the above regression.
