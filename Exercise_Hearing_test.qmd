...
pacman::p_load(downloadthis, ggstatsplot, haven, hrbrthemes, interactions, kableExtra, knitr, labelled, lm.beta, modelsummary, tidyverse, ggpubr, quarto, openxlsx)
...
pacman::p_load(haven, ggstatsplot, hrbrthemes, interactions, kableExtra, knitr, labelled, lm.beta, modelsummary, openxlsx, tidyverse)
...
Comparing the models show that the addition of gender and the genderxage interaction only marginally increases the internal model fit: adjusted R² value from `r round(as.numeric(model_summary_results@data[7,2]), digits = 2)` (Model 1) to `r round(as.numeric(model_summary_results@data[7,8]), digits = 2)` (Model 3).
:::
...
Below, we need to explicitly convert the variable *Gender* into a a labelled variable. This is because the PROCESS macro requires the moderator to be a numeric variable. As a result, the variable *Gender* now is coded numerically with 1 (females) and 2 (males). We store the results of this transformation to a dataset called *d_process*.

Subsequently, we run the PROCESS macro. The function requires the data set (*data*), the dependent variable (*y*), the independent variable (*w*), the moderator (*w*), the model number (which is 1, see, [@Hayes.2022, p. 621 for an overview]), the centering method, the number of bootstrap samples (*boot*), and the confidence level (*conf*). We further set the seed to ensure reproducibility. In addition, we set the progress bar to FALSE.
...
For the interpretation, we should first focus on the p-value of the t-test for the model terms (**Pr(\>\|t\|)**). If one of both variables is accompanied by a value of \< 0.05, we would conclude that this variable has a higher explanatory power in hearing capabilities. If both predictors are below 0.05, the predictor with the highest magnitude in the standardized regression coefficients (**Standardized**) explains better. However, suppose both predictors did not reach significance. In that case, none of them is a good (in a statistical sense) predictor of hearing capabilities, and we should not interpret differences in the magnitude of the standardized coefficients. Our results support the notion that *Age* hase ahigher influence in explaining hearing capabilities.
:::
