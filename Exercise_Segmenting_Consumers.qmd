...
subtitle: "Exercise 5 - Segmenting consumers based on sensory acceptance tests in sensory labs, immersive environments, and natural consumption settings"
author: 
  - name: 
      given: Marcel
      family: Lichters
...
citation:
  language: "en"
  type: post
  url: https://rpubs.com/M_Lichters/smpiexercise5
  title: 'Sensory Marketing & Product Innovation: Exercise 5 - Segmenting consumers based on sensory acceptance tests in sensory labs, immersive environments, and natural consumption settings'
...
4.  The third line uses the function *p_load()* from the pacman package to install (if necessary) and loads all packages that we provide as arguments (e.g., [rstatix](https://cran.r-project.org/web/packages/rstatix/index.html) [@rstatix], which provides functions for performing basic statistical tests}

    if (!"pacman" %in% rownames(installed.packages())) install.packages("pacman")

    library(pacman)

    pacman::p_load(tidyverse, osfr, r
...
Thus, in the article, the authors applied the Bonferroni correction [@Bonferroni.1936] to account for multiple group comparisons. Researchers in statistics have proposed countless [alternatives for post-hoc tests](https://www.statology.org/anova-post-hoc-tests/) to account for family-wise Alpha error when comparing multiple groups (in our case, the 3 environments). For illustration, we choose Holm's method [@Holm.1979] for the code chunk below, which is less conservative (i.e., differences get significant faster) as compared to Bonferroni .
