...
citation:
  language: "en"
  type: post
  url: https://rpubs.com/M_Lichters/smpichap6
...
pacman::p_load(tidyverse, SensoMineR, purrr, sensR)
...
We use the R Commander GUI. Thus, the subsequent code only shows how to install and load the R Commander [@Rcmdr; @fox2005] and the package *SensoMineR* [@SensoMineR-2; @le2008]. The rest of this example is presented in the lecture slides.
:::
...
if (!"pacman" %in% rownames(installed.packages())) install.packages("pacman") ##install pacman if not present

if (!"Rcmdr" %in% rownames(installed.packages())) install.packages("Rcmdr") ##install R Commander if not present

library(pacman) ##loads pacman package

pacman::p_load(tidyverse, and) ##load packages (and firstly install if necessary) 

set_language("en_US") ##set language to English (important for R Commander GUI) 

source("http://sensominer.free.fr/install-senso.r") ##latest version of SensoMineR and R Commander PlugIn, also opens R Commander
