...
# Examplary Analyses for Descriptive analysis

::: callout-warning
## Attention

Make sure, that the package *SensoMineR* [@SensoMineR-2; @le2008] has been loaded (see above).
:::
...
The data used here refer to six varieties of LINDT chocolates sold in France [@pagès2001].
...
Let's have a look to the *sensochoc* data:
...
Get an overview over the mean values per product x descriptor
...
Next, we test, which descriptors are significantly discriminating between products
...
What we see here is a PCA-Plot (based on a principal component analysis) of the percepual space of the trained panel.
We see all descriptors as vectors. Two descriptors are highly positively correlated if the angle between them is very small. Two perfectly negative correlated descriptors would point in opposite directions (e.g., have a look to *Acidity* vs. *Sweetness*). Further we see, how much information of the panel's product evaluations is represented by the first two principal components. As a general rule of thumb, both axes should esxplan at minimum 75-80% of the information contained in the panelists' perceptions. Otherwise a 2D map is may miss important information on the products' dissimilarity. The length of each vector furthermore correspondents to the *communality* in the PCA. Put different, descriptors possessing short vectors are now well explained in terms of th first two principal components.
:::

The map above helps to understand which sensory descriptors usually go hand in hand for the evaluated product category.
...
This graphic is very similar to the first one. It, again, presents the first two principal components in the space of descriptors. This time, however, the plot also provides an impression of the uncertainty, or better said, the (dis)agreement in the panalists' scoring in relation to each descriptor. Here, we can see that the panel's agreement is very good for some descriptors (e.g., *Sweetness*), while it is rather low for others as indicated by a wide cloud of individual evaluations (e.g., *Sticky*).

:::

Product developers should trust more in the panel's ability to use descriptors with low variance (vs. high variance).

We move on with the next plot:
...
This biplot provides us with information on the product's location in the space of the sensory perception as defined by the panel. More precisely, we see the factor/ component scores of each product. The logic is that products that are very far away from each other on a certain axis are very dissimilar from a sensory perception. Products that are very close to each other are very similar in terms of sensory descriptors. 

:::


In practice, this information is essential to understand the competitive landscape. It provides companies with information on sensory (dis)similarity of own products in the assortment , R&D samples and also competitors. **Empty spaces** in the product perception map may also give raise to product innovation. 


Finally, we can enrich the above data visualization in terms of uncertainty estimates in form of 95% confidence ellipses.
...
This helps us to understand which products do, overall, significantly differ from each other in terms of sensory descriptors and which not. For example, the confidence ellipse for Exellence Noir 70% does not overlap with other products. The product is therefore very sensory different from other product variants. This is not true for the three products at the bottom of the plot, which have a lot of overlap.


Finally, we build a bridge between the perceptions of the trained panel and the naaive consumer liking judgement by superimposing a preference mapping of distinct consumer segments.
...
Interestingly, we see two bigger red areas of preference in the heat map produced by preference mapping
