...
sensochoc %>% 
  as_tibble() %>% 
  group_by(Product) %>% 
  summarise(across(where(is.numeric), ~ round(mean(.x, na.rm = TRUE), digits = 2))) %>% t() 
...
# results$eig # # # # dev.new() # barplot(results$eig\[,1\], main="Eigenvalues", xlab="Dimension",

# ylab="Eigenvalues",names.arg=1:nrow(results$eig)) # results$coordinates

# results$correl # results$hotelling

# coltable(results\$hotelling, main.title =
