...
<a title="Chemnitz University of Technology" href="https://www.tu-chemnitz.de/wirtschaft/fakultaet/studiengaenge.php.en">[![Chemnitz University of Technology](TU_Chemnitz_Logo.png){alt="Chemnitz University of Technology" width="7.7cm"}](https://www.tu-chemnitz.de/wirtschaft/fakultaet/studiengaenge.php.en)</a>
...
pacman::p_load(
  haven, kableExtra, knitr,
  labelled, tidyverse, remotes
)

if (!"sensR" %in% rownames(installed.packages())) install_version("sensR", "1.5-2", repos = "https://cran.rstudio.com/")
...
1.  The code chunk below first evaluates if the package [pacman](https://cran.r-project.org/web/packages/pacman/index.html) [@pacman] is already installed to your machine. If yes, the corresponding package will be loaded. If not, *R* will install the package. Alternatively, you can do this manually first by executing *install.packages("pacman")* and then *library(pacman)*.

2.  The second chunk then loads the package [pacman](https://cran.r-project.org/web/packages/pacman/index.html).

3.  The third chunk uses the function *p_load()* from the pacman package to install (if necessary) and load all packages that we provide as arguments (e.g., [haven](https://cran.r-project.org/web/packages/haven/index.html) [@haven], which provides functions to import foreign statistical data formats into *R*).

4.  Some packages we will use are discontinued from being supported on CRAN. This is the case for [sensR](https://cran.r-project.org/web/packages/sensR/index.html) [@sensR]. Thus, the fourth chunk first checks whether sensR is already installed. IF not, the function *install_version()* from the package [remotes](https://cran.r-project.org/web/packages/remotes/index.html) [@remotes] is used to install the last supported version.

5.  Finally, the last command loads the sensR package.

```{r}
#| eval: false

if (!"pacman" %in% 
    rownames(installed.packages())) install.packages("pacman")
...
if (!"sensR" %in% rownames(installed.packages())) install_version("sensR", 
                                                                  "1.5-2", 
...
In most simplest case, analysis draws on a $\chi^{2}$-test (without continuity correction). Here, the test statistic is calculated as follows:
...
In *R*, for example the package [sensR](https://cran.r-project.org/web/packages/sensR/) is providing functions to handle nearly all types of discrimination tests, including Triangle tests [@naes2010, chap. 7]. We will use the function **discrim()**. This function allows us to specify many arguments (see help by pressing 'F1'). We will do so for

-   correct = the number of correct answers observed
...
-   test = Whether to conduct a "difference" or "similarity" test.

We assign the results of our Discrimination Triangle test to a new object named 'triangle_test'. Then we call for its content.
