...
pacman::p_load(tidyverse, SensoMineR, sensR, pwr, downloadthis, openxlsx, labelled, lm.beta, ggstatsplot, jtools)
...
pacman::p_load(tidyverse, SensoMineR, sensR, pwr, openxlsx, labelled, lm.beta, ggstatsplot, jtools)
...
A Triangle test protocol would have a higher power of `r round(power_triangle*100, digits = 2)`%. Therefore, if the observed effect is true in the underlying population and Jägermeister can not afford to have a larger sample size than n=206, the company should switch from a Duo-trio to a Triangle test protocol.
:::
...
Now, we set up Model 1 in PROCESS. While doing this, one caveat is that the process macro is not working with nominal-scaled variables stored as characters, such as the *Gender* variable in our case. We first transform this variable as a workaround to a labeled numerical one.

```{r}

hearing_test$Gender <- hearing_test$Gender %>% to_factor() %>% to_labelled()
...
From the results above, we can see that the decline in hearing capabilities does not hinge on the gender of persons. This is evinced by the statistically insignificant interaction term of of *Gender* $\times$ *Age* (see the p value for the row **Int_1**). This is further supported by the bootstrapping analysis. Here, the 90% confidence intervals for the model term **Int_1** are crossing **0**.
:::

## Task 3 - Which variable, age or gender, impacts hearing capabilities more in the model without the interaction term?

The most easiest way to evaluate the explanatory power of different predictors in the regression model is to compare their standardized regression coefficient's magnitude qualitatively. This assumes that, firstly, Age's and Gender's main effects are both signigificantly different from 0 (p\<0.05). To get a summary of the standardized regression coefficients, we apply the function *lm.beta()* from the *lm.beta* package [@lm.beta]. First, we build a model with the main effects of *Age* and *Gender* only.
...
For the inpretetion we should first focus on the p-value of the t-test for the model terms (**Pr(\>\|t\|)**). If one of both variables is accompanied by a value of \< 0.05, we would conclude that this variable has a higher explanatory power in hearing capabilities. If both predictors are below 0.05, then the predictor with the highest magnitude in the standardized regression coefficients (**Standardized**) explains better. However, if both predictors did not reach significance, then non of them is a good (in a statistical sense) predictor of hearing capabilities and we shuld not interpret differences in the magnitude of the standardized coefficients.
:::

## Task 4 - How much total information in hearing capabilities can be explained by gender, age, and its interaction?

This question is directed to the adjusted R² value for the corresponding regression model. We build one with both main effects and the interactionterm between *Age* and *Gender*.
...
As always, there multiple routes in *R* that lead to this result. In the following, I use the function *ggscatterstats()* provided by the package *ggstatsplot* [@patil2021].
