...
subtitle: "Exercise 2 - Making Choices"
author: 
  - name: 
      given: Marcel
      family: Lichters
...
citation:
  language: "en"
  type: post
  url: https://rpubs.com/M_Lichters/smpiexercise2 
  title: 'Sensory Marketing & Product Innovation: Exercise 2 - Making Choices'
...
R analysis script presenting the solutions for the exercise in Sensory Marketing regarding Biswas et al. [-@biswas2014]. In one of the questions, students are asked to estimate the appropriate sample size for a replication of Study 1b's findings (under $\chi^{2}$-test, $\alpha$ = 5%, Power = 0.8).
...
To estimate a minimum sample size n for the replication, we apply the *pwr.chisq.test()* function from the [pwr](https://cran.r-project.org/web/packages/pwr/index.html) package [@pwr]. This function needs us to provide 5 arguments to fill its parameters. These are (see help by pressing 'F1'):
...
We can see that the calculated sample size `r ceiling(results$N)` perfectly mirrors the one we have seen in G\*Power 3 [@faul2007]. Keep in mind: if an original study comes with a medium effect size, then a replication with a power of 80% often needs a comparable sample size as compared to the original study. However, studies reporting only small effects usually need many more participants for a successful replication.
...
We assume that the observed effect size Cohen's $\omega$ = `r round(Cohen_omega, digits = 4)` is the true effect size to find. Strictly speaking, this is wrong. Our calculated value is only a sample estimate of the true effect size in the underlying population. Under the assumption of knowing the true effect size $\omega$ = `r round(Cohen_omega, digits = 4)` we calculated that we need at least n=`r round(results$N, digits=0)` participants for a replication of the original findings with a power of 80%. This means that we will have approximately the chance of 80% to find the effect to be significant at $\alpha$=0.05. Because, the true effect to find might, however, be smaller, this calculation may present an overly optimistic approach. Therefore, other researchers prefer to focus on the confidence intervals for the estimate of effect size measures [@thompson2002].

Apart from Cohen's $\omega$, there are plenty of other effect sizes 'out in the market'. They all can be converted into each other[@fern1996; @borenstein2009; @lenhard2017; @volker2006]. In our case (Chi² with *df*=1), for example, Cohen's $\omega$ is equivalent to the common language effect size *r* [@lenhard2017].

One *R* package that facilitates the calculation of confidence levels for effect sizes is the [compute.es](https://cran.r-project.org/web/packages/compute.es/) package [@compute.es]. This package supports the use of *r* as an effect size measure.
...
We can see that the more conservative sample size is `r ceiling(results_worst_case$N)`, which makes a replication of the original findings almost impossible.
