...
citation:
  language: "en"
  type: post
  url: https://marcel-lichters.quarto.pub/smpiexercise1/
...
R analysis script presenting the solutions for the exercise in Sensory Marketing regarding Biswas et al. [-@biswas2014]. In one of the questions, students are asked to estimate the appropriate samples size for a replication of Study 1b's findings (under $\chi^{2}$-test, $\alpha$ = 5%, Power = 0.8).
...
How many subjects have to be recruited in order to replicate the overall effect of Study 1b (Assumptions: $\alpha$=5%, Power=0.8, Chi²-test, effect size = sample estimate)?
...
This is all that we need to first calculate the observed effect size Cohen's $\omega$ [@cohen2013]. Remember from exercise: Cohen's $\omega$=$\sqrt{\frac{chi^{2}}{n}}$. Therefore, we simply need to solve $\sqrt{\frac{7.07}{51}}$
